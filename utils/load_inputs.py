...
class load_inputs():
	...
	def exoplanet(self,so):
		so.exo.radius = np.float(so.exo.radius)
		if so.run.mode == 'transmission':
			spec = np.loadtxt(so.input_path + so.exo.folder + so.exo.name)
		...
		if so.run.mode =='Helium': # define so.exo.s in terms of excess flux
			stel_rad  = so.const.rsun * so.const.radii[np.where(so.const.types == so.stel.type)[0]]
			so.exo.v = np.arange(1075,1090,.01)  
			sig = 0.1
			cent = 1083.0
			amp = 0.012 # excess flux

			# if (Rp+H)^2 = Rp^2 + X*Rs^2 where X is excess flux then
			H = np.sqrt(so.exo.radius**2 + amp * stel_rad**2) - so.exo.radius
			s = np.exp(-.5*((so.exo.v - cent)/sig)**2)/(sig * np.sqrt(2*np.pi))
			
			so.exo.s = so.exo.radius + H*s/np.max(s) # this is in excess absorption, must multiple by (rp/r2 )^2

		if so.run.mode =='Potassium': # define so.exo.s in terms of excess flux
			stel_rad  = so.const.rsun * so.const.radii[np.where(so.const.types == so.stel.type)[0]]
			so.exo.v = np.arange(755,785,.01)  
			sig  = 0.06
			cent = 770.0
			amp  = 0.005 # excess flux
		...
		if so.run.mode =='Potassium_model': # define so.exo.s in terms of excess flux
			spec = np.loadtxt(so.input_path + so.exo.folder + so.exo.name)
			x,y   =  spec[:,0], spec[:,1] # x: micron vacuum  y: unitless Rp2/Rs2
			rstar    = 1.27 *so.const.rsun  #; Stellar radius (default units: radunits)
			rplanet  = 1.00 *so.const.rjup  #; Planetary radius (default units: radunits)
			
			v = vac_to_stand(x[::-1]*10000)/10 # needs to be in angstrom for vac to air fxn
			s = np.sqrt(y[::-1]*rstar**2)

			# resample
			so.exo.v       = np.arange(np.round(np.min(v)),np.round(np.max(v)),.01)
			tck_exo        = interpolate.splrep(v,s, k=2, s=0)
			so.exo.s       = interpolate.splev(so.exo.v,tck_exo,der=0)
	...
	def telluric(self,so):
		# OH 
		f = np.loadtxt(so.input_path + so.tel.folder + so.tel.name_oh) # vacuum wl angstroms
		...
		# TAPAS oxygen
		tapload        = fits.open(so.input_path + so.tel.folder + so.tel.name_o2)
		wavo2_all      = 1e7/tapload[1].data['wavenumber']  # convert to standard*****                                                     
		isub           = np.where( (wavo2_all > np.min(so.exo.v)) & (wavo2_all < np.max(so.exo.v)))[0]

		wavo2         = wavo2_all[isub][::-1]
		o2             = tapload[1].data['transmittance'][isub][::-1]
		so.tel.airmass = float(tapload[1].header['AIRMASS'])
		tapload.close()
		wavo2          = vac_to_stand(wavo2*10)/10 # must be in angstroms
		...
		# TAPAS water
		tapload         = fits.open(so.input_path + so.tel.folder + so.tel.name_h2o)
		wavh2o_all          = 1e7/tapload[1].data['wavenumber']  # convert to standard*****                                                     
		isub = np.where( (wavh2o_all > np.min(so.exo.v)) & (wavh2o_all < np.max(so.exo.v)))[0]

		wavh2o          = wavh2o_all[isub][::-1]
		h2o             = tapload[1].data['transmittance'][isub][::-1]
		...
		tapload.close()
		wavh2o                    = vac_to_stand(wavh2o*10)/10
		h2o[np.where(h2o < 0)[0]] = 1e-8
	
		h2o_interp = interp1d(wavh2o,h2o,bounds_error=False,fill_value=1)
		so.tel.h2o  = h2o_interp(so.exo.v)
	
		# Tapas airmass at zenith REDO- download larger raleigh file
		rayleigh     = np.loadtxt(so.input_path + so.tel.folder + so.tel.name_rayleigh)	
	...
	def filter(self,so,nbands=0):
		...
		filt = np.loadtxt(so.input_path + so.filt.folder + so.filt.dualon_raw_name)
		...
		f        = np.loadtxt(so.input_path + so.filt.folder + so.filt.name_alluxa)
		on_lam   = 759.6 # nm
		off_lam  = 758.0 # nm
		...
		# I band filter	
		so.filt.I_raw = np.loadtxt(so.input_path + so.filt.folder + so.filt.name_I)
	...
	def stellar(self,so):
		...
		so.stel.name = glob.glob(so.input_path + so.stel.folder + so.stel.type + '*.fits')[0]
		...
		# what's the integrated i flux supposed to be in photons?
		xstel              = Iband_sub[0]  # angstroms
		so.filt.dl_l_I     = np.mean(integrate(xstel,I_interp(Iband_sub[0]))/xstel)
		nphot_expected_0   = so.filt.dl_l_I * so.filt.zp_I * 10**(-0.4*so.stel.I_mag) * 1.51*10**3 # last factor to convert to phot/s/cm2
		nphot_phoenix      = integrate(xstel,I_stel_air)
		so.stel.factor_0   = nphot_expected_0/nphot_phoenix
	
		# Save stellar spectrum info
		oxy_sub     = load_phoenix(so.stel.name,wav_start=np.min(so.exo.v),wav_end=np.max(so.exo.v)) 
